...
import tensorboard
from conv1dClassify import Conv1DNet, Classifier, Conv1DAutoencoder
...
def train_test_model(device, model, train_set, test_set, criterion, optimizer, train_ratio=0.7, num_epochs=80000):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            # Iterate over data.
            batch_avg_EER = 0
            for batch_id, (utterance, noise) in enumerate(dataloader):
                # forward
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        utterance = torch.reshape(utterance, (num_of_speakers * num_of_utters, num_of_features)).to(
                            device)
                        piezos = utterance[:, :1976].to(device)
                        audios = utterance[:, 1976:].to(device)

                        embeddings, out_audio = model(piezos)
                        ...
                        loss_audio = calculate_l2_distance(audios, out_audio).mean()
                        loss = loss_ge2e + 0.0001 * loss_audio
                        # loss = loss_audio
                    ...
                    if phase == 'test':
                        ...
                        enrollment_embeddings, enrollment_audios = model(enrollment_batch[:, :1976].to(device))
                        verification_embeddings, verification_audios = model(verification_batch[:, :1976].to(device))
...
if __name__ == "__main__":
    ...
    train_test_model(device, model, train_set, test_set, criterion, optimizer)
    torch.save(model.state_dict(), 'model_auth.pth')
