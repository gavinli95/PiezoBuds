                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        utterance = torch.reshape(utterance, (num_of_speakers * num_of_utters, w, l)).to(
                            device)
                        utterance = torch.transpose(utterance, 2, 1).to(device)
                        piezos = utterance[:, :, :w//2].to(device)
                        audios = utterance[:, :, w//2:].to(device)

                        embeddings = model(piezos)
                    ...
                    if phase == 'test':
                        ...
                        enrollment_batch = torch.transpose(enrollment_batch, 2, 1).to(device)
                        verification_batch = torch.transpose(verification_batch, 2, 1).to(device)
                        ...
                        enrollment_embeddings = model(enrollment_batch[:, :, :w//2].to(device))
                        verification_embeddings = model(verification_batch[:, :, w//2:].to(device))
...
if __name__ == "__main__":
    ...
    print(model)
    train_set = STFTFeatureSet(True, train_ids, 20)
    test_set = STFTFeatureSet(True, valid_ids, 10)
    ...
    torch.save(model.state_dict(), 'model_auth_siamese.pth')
