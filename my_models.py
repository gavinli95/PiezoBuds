...
class SpeechEmbedder(nn.Module):
    ...
    def __init__(self, n_mels=80, output=128):
        super(SpeechEmbedder, self).__init__()
        self.LSTM_stack = nn.LSTM(n_mels, 512, num_layers=3, batch_first=True)
        for name, param in self.LSTM_stack.named_parameters():
            if 'bias' in name:
                nn.init.constant_(param, 0.0)
            elif 'weight' in name:
                nn.init.xavier_normal_(param)
        self.projection = nn.Linear(512, output)
...
class UNet2D(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.enc1 = EncoderBlock(in_channels, 16, use_pooling=True)
        self.enc2 = EncoderBlock(16, 32, use_pooling=True)
