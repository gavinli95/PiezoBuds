...
def train_test_model(device, model, dataset, criterion, optimizer, train_ratio=0.7, num_epochs=8000):
...
if __name__ == "__main__":
    ...
    root_piezo = './frequencybins/{}_piezo.npy'
    root_audio = './frequencybins/{}_audio.npy'
    ...
    piezos_divide_audios = np.divide(piezos, audios)

    if mode == '0':
        model = Conv1DNet(83).to(device)
        dataset = LatentFeatureSet(piezos, labels)
    if mode == '1':
        model = Conv1DNet(83).to(device)
        dataset = LatentFeatureSet(audios, labels)
    if mode == '2':
        model = Conv1DNet(166).to(device)
        dataset = LatentFeatureSet(piezos_audios, labels)
    if mode == '3':
        model = Conv1DNet(83).to(device)
        dataset = LatentFeatureSet(piezos_divide_audios, labels)
