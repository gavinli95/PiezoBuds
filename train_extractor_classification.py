...
if __name__ == "__main__":
    ...
    data_file_dir = './processed_data/power_spectra/res_256_hop_256_t_160/' # folder where stores the data for training and test
    ...
    comment = 'old_model_out_hop_256_b_2_u_20_class_pwr_spec_42u' # simple descriptions of specifications of this model, for example, 't_f' means we use the model which contains time and frequency nn layers

    if train_seperate == False:
        a_res = 256
        p_res = 256
        # extractor = extractor1DCNN(device=device, channel_number=160, in_feature=256, out_feature=128).to(device)
        extractor = torchvision.models.resnet34(pretrained=True)
        extractor.conv1 = nn.Conv2d(2, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        extractor.fc = nn.Linear(extractor.fc.in_features, 42)
        # extractor = SpeechEmbedder().to(device)
        extractor.to(device)
        class_model = class_model_weighted(device=device).to(device)
    ...
    extractor = train_and_test_model(device=device, model=extractor, class_model=class_model,
