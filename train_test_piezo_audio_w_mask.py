...
from my_models import CNNModel, LSTMWithAttention, GE2ELoss, ConverterNetwork, ConverterNetwork1DCNN, DeepSet, BinaryMask
import tensorboard
...
def train_and_test_model(device, model, mask, ge2e_loss, is_cnn, train_set, test_set, optimizer, optimizer_mask, num_epochs=2000):
    ...
    for epoch in range(num_epochs * 2):
        ...
        for phase in ['train', 'test']:
            ...
            for batch_id, (utterance, ids) in enumerate(dataloader):
                ...
                with torch.set_grad_enabled(phase == 'train') and torch.autograd.set_detect_anomaly(True):
                    ...
                    if phase == 'train':
                        ...
                        embeddings_piezo, _ = model(piezos)
                        embeddings_audio, _ = model(audios)
                        embeddings_x, _ = model(mask(piezos, audios))
                        ...
                        loss_x, loss_aa = ge2e_loss(embeddings_audio, embeddings_x)
                        loss_extractor = loss_aa + loss_pp + loss_x
                        loss_avg_batch_extractor += loss_extractor.item()
                        # mask
                        centroids_x = get_centroids(embeddings_x)
                        sim_matrix_px = get_modal_cossim(embeddings_piezo, centroids_x)
                        sim_matrix_ax = get_modal_cossim(embeddings_audio, centroids_x)
                        loss_mask_px, _ = cal_intra_loss(sim_matrix_px, device)
                        loss_mask_ax, _ = cal_intra_loss(sim_matrix_ax, device)
                        loss_mask = torch.abs(loss_mask_px + loss_mask_ax) + torch.abs(loss_mask_px - loss_mask_ax)
                        loss_avg_batch_mask += loss_mask.item()

                        loss = loss_extractor + 3 * loss_mask # not used when applied with below codes
                        ...
                        if int(epoch / 20) % 2 == 0:
                            ...
                            loss_mask.backward()
                            torch.nn.utils.clip_grad_norm_(mask.parameters(), 1.0)
                            optimizer_mask.step()
                        ...
                        enrollment_x = mask(enrollment_piezos, enrollment_audios)
                        verification_x = mask(verification_piezos, verification_audios)
                        # get centroids from enrollment
                        embeddings_piezo, _ = model(enrollment_piezos)
                        _, output_dim = embeddings_piezo.shape
                        embeddings_piezo = torch.reshape(embeddings_piezo, (num_of_speakers, num_of_utters // 2, output_dim))
                        embeddings_audio, _ = model(enrollment_audios)
                        embeddings_audio = torch.reshape(embeddings_audio, (num_of_speakers, num_of_utters // 2, output_dim))
                        embeddings_x, _ = model(enrollment_x)
                        ...
                        embeddings_piezo, _ = model(verification_piezos)
                        embeddings_piezo = torch.reshape(embeddings_piezo, (num_of_speakers, num_of_utters // 2, output_dim))
                        embeddings_audio, _ = model(verification_audios)
                        embeddings_audio = torch.reshape(embeddings_audio, (num_of_speakers, num_of_utters // 2, output_dim))
                        embeddings_x, _ = model(verification_x)
                        embeddings_x = torch.reshape(embeddings_x, (num_of_speakers, num_of_utters // 2, output_dim))
            ...
            if phase == 'train':
                ...
                epoch_loss_mask = loss_avg_batch_mask / num_of_batches
                print(f'{phase} Loss Mask: {epoch_loss_mask:.4f}')
                # writer.add_scalar('Loss/train', epoch_loss, epoch)
                wandb.log({'epoch': epoch, 'Loss/train_mask': epoch_loss_mask})
            else:
                ...
                print("\nCentroids: X  Verification Input: X "
                              "\nEER : %0.2f (thres:%0.2f, FAR:%0.2f, FRR:%0.2f)" % (EERs[-1], EER_threshes[-1], EER_FARs[-1], EER_FRRs[-1]))
                wandb.log({'epoch': epoch, 'EER/C_X': EERs[-1], 'FAR/C_X': EER_FARs[-1], 'FRR/C_X': EER_FRRs[-1]})
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        ...
        lr = float(sys.argv[4])
    mask = BinaryMask(device).to(device)
    ...
    model, converter = train_and_test_model(device, model, mask, ge2e_loss, is_CNN, train_set, test_set, optimizer, optimizer_mask, num_of_epoches)
