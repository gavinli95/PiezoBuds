...
def train_and_test_model(device, extractor, loss_func, data_set, optimizer, train_batch_size, test_batch_size,
            ...
            for batch_id, (piezo_clips, audio_clips, ids) in enumerate(dataloader):
                ...
                with torch.set_grad_enabled(phase == 'train') and torch.autograd.set_detect_anomaly(True):
                    ...
                    if phase == 'train':
                        ...
                        combined = torch.cat((piezos, audios), dim=1)
                    ...
                    if phase == 'test':
                        ...
                        pred_ids_combined = extractor(combined)
                        embeddings_fusion = torch.clone(hooked_output)
                        embeddings_fusion.contiguous()
                        embeddings_fusion = embeddings_fusion.view(batch_size, n_uttr, -1)
                        embeddings_enroll, embeddings_veri = torch.split(embeddings_fusion, n_uttr // 2, dim=1)
...
if __name__ == "__main__":
    ...
    # load the data 
    data_set = WavDatasetForVerification('./processed_data/wav_clips/piezobuds/', list(range(n_user)), 40)
    print(len(data_set))

    loss_func = nn.CrossEntropyLoss()

    extractor = train_and_test_model(device=device, extractor=extractor, loss_func=loss_func, data_set=data_set, optimizer=optimizer,
