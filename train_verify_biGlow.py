            ...
            for batch_id, (piezo_clips, audio_clips, ids) in enumerate(dataloader):
                ...
                with torch.set_grad_enabled(phase == 'train') and torch.autograd.set_detect_anomaly(True):
                    ...
                    if phase == 'train':
                        ...
                        z_outs = converter.reverse(z_outs, reconstruct=False)
                        ...
                        loss_extractor = loss_a + loss_p + loss_conv + logdet_i + logdet_c
                        # if epoch >= epoch_th:
                    ...
                    if phase == 'test':
                        ...
                        tmp_converter = biGlow(in_channel=3, n_flow=4, n_block=3).to(device)
                        ...
                        with torch.set_grad_enabled(True) and torch.autograd.set_detect_anomaly(True):
                            for e in range(1):
                                ...
                                z_outs = tmp_converter.reverse(z_outs, reconstruct=False)
                                z_outs = z_outs.contiguous()
...
if __name__ == "__main__":
    ...
    comment = 'ecapatdnn_w_biGlow_wo_enroll_mse_logdet'.format(n_fft, hop_length)
    ...
    converter = biGlow(in_channel=3, n_flow=4, n_block=3).to(device)
